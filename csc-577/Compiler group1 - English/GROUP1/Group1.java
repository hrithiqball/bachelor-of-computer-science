...
class Group1 {
    ...
    void atom(String cls, MutableInt left, MutableInt right, MutableInt result, MutableInt comp, MutableInt dest) {
    ...
    void Stmt() {
        ...
        else if (token.get_class() == Tok.DO) DoWhileStmt();  // Added DoWhileStmt
    ...
    void DoWhileStmt() {
        ...
        else err("expected 'while' in do-while statement");
        if (token.get_class() == Tok.LPar) token.getToken();
        else err("expected left parenthesis in do-while statement");
        BoolExpr(l2);
        if (token.get_class() == Tok.RPar) token.getToken();
        else err("expected right parenthesis in do-while statement");
        if (token.get_class() == Tok.SEMI) token.getToken();
        else err("expected ';' at end of do-while statement");
        atom("JMP", zero, zero, zero, zero, l1);
        atom("LBL", zero, zero, zero, zero, l2);
    }
